...
export class AuthorBlogComponent implements OnInit {
  ...
  ngOnInit() {
    this.headerService.sendTitle("Agenda de Autores");
    this.nameWindow = "Autores";
    ...
    this.gridList.config.getService = "/author-blog/search";
    this.gridList.config.entity = "Autores";
  }
  get f() {
  ...
  actionAnswer(event) {
    ...
    switch (event.action) {
      case "edit": {
        ...
        this._authorService.getById(event.field).subscribe(
          infoData => {
  ...
  deletItem() {
    this._middleService.sendLoading(true);
    this._authorService.delete(this.idContactEmail).subscribe(
      data => {
        this._middleService.sendLoading(false);
        this._middleService.sendMessage(
          this.nameWindow,
        ...
        this._authorService
          ...
          .subscribe(
            infoSave => {
              ...
              this._middleService.sendMessage(
                this.nameWindow,
                "EL autor ha sido creado correctamente",
                "ok"
              );
              this.close();
              this.gridList.getInfo();
