...
export class DynamicContentPageComponent implements OnInit {
  ...
  listImageField = {} as any;
  listImageDimension = {} as any
  @Input() config: any;
  @Input() objParent: any;
  ...
  addArraySection(content, newSection) {
    ...
    let objClone = JSON.parse(JSON.stringify(this.contentArray.array[0]));
    objClone.value = undefined;
    objClone.attribute.forEach(element => {
      element.value = "";
    });
    this.contentArray.array.push(objClone);
  }
  ...
  showWindowMultimedia(content, position?, maxSize?, maxDimension?) {
    if (!position && position !== 0) {
      ...
      this.showContent.emit({ content, position, maxSize, maxDimension });
    }
  }
}
